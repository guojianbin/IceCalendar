//        <div>
//            <input type="text" name="time1" id="time1" onclick="IceCalendar.SetDate(this)" value="2006-8-1" />
//            <input type="text" name="time2" id="time2" value="2006-8-1" /><input name="" type="button"
//                onclick="IceCalendar.SetDate(this,document.getElementById('time2'))" value="选择" />
//        </div>
//  
//  纯净的js日期控件
//
...
Ice_calendar.prototype = {
    ...
    GetDateLayer: function () {
        return window.document.getElementById("L_DateLayer").contentWindow;
    ...
    CreateHTML: function() {
        ...
        htmlstr += "<div id=\"Ice_calendar-year-month\"><div id=\"Ice_calendar-PrevM\" onclick=\"parent." + this.NewName + ".PrevM()\" title=\"前一月\"><b>&lt;</b></div><div id=\"Ice_calendar-year\" onmouseover=\"style.backgroundColor='#86D700'\" onmouseout=\"style.backgroundColor='white'\" onclick=\"parent." + this.NewName + ".SelectYearInnerHTML('" + this.L_TheYear + "')\"></div><div id=\"Ice_calendar-month\"  onmouseover=\"style.backgroundColor='#86D700'\" onmouseout=\"style.backgroundColor='white'\" onclick=\"parent." + this.NewName + ".SelectMonthInnerHTML('" + this.L_TheMonth + "')\"></div><div id=\"Ice_calendar-NextM\" onclick=\"parent." + this.NewName + ".NextM()\" title=\"后一月\"><b>&gt;</b></div></div>\r\n";
        htmlstr += "<div id=\"Ice_calendar-week\"><ul  onmouseup=\"StopMove()\"><li>日</li><li>一</li><li>二</li><li>三</li><li>四</li><li>五</li><li>六</li></ul></div>\r\n";
        htmlstr += "<div id=\"Ice_calendar-day\">\r\n";
        htmlstr += "<ul>\r\n";
        for (var i = 0; i < this.L_WDay.length; i++) {
            htmlstr += "<li id=\"Ice_calendar-day_" + i + "\" style=\"background:#e0e0e0\" onmouseover=\"this.style.background='#86D700'\"  onmouseout=\"this.style.background='#e0e0e0'\"></li>\r\n";
        }
        htmlstr += "</ul>\r\n";
        htmlstr += "<span id=\"Ice_calendar-today\" onclick=\"parent." + this.NewName + ".Today()\"><b>今天</b></span>\r\n";
        htmlstr += "</div>\r\n";
        //htmlstr+="<div id=\"Ice_calendar-control\"></div>\r\n";
        htmlstr += "</div>\r\n";
        ...
        htmlstr += "document.getElementById(\"Ice_calendar-week\").onmousedown=function(e){\r\n";
        ...
        stylestr += "#Ice_calendar{border:1px solid blue;width:158px;padding:1px;height:180px;z-index:9998;text-align:center}\r\n";
        stylestr += "#Ice_calendar-year-month{height:23px;line-height:23px;z-index:9998;}\r\n";
        stylestr += "#Ice_calendar-year{line-height:23px;width:60px;float:left;z-index:9998;position: absolute;top: 3; left: 19;cursor:default}\r\n";
        stylestr += "#Ice_calendar-month{line-height:23px;width:48px;float:left;z-index:9998;position: absolute;top: 3; left: 78;cursor:default}\r\n";
        stylestr += "#Ice_calendar-PrevM{position: absolute;top: 3; left: 5;cursor:pointer}"
        stylestr += "#Ice_calendar-NextM{position: absolute;top: 3; left: 145;cursor:pointer}"
        stylestr += "#Ice_calendar-week{height:23px;line-height:23px;z-index:9998;}\r\n";
        stylestr += "#Ice_calendar-day{height:136px;z-index:9998;}\r\n";
        stylestr += "#Ice_calendar-week ul{cursor:move;list-style:none;margin:0px;padding:0px;}\r\n";
        stylestr += "#Ice_calendar-week li{width:20px;height:20px;float:left;;margin:1px;padding:0px;text-align:center;}\r\n";
        stylestr += "#Ice_calendar-day ul{list-style:none;margin:0px;padding:0px;}\r\n";
        stylestr += "#Ice_calendar-day li{cursor:pointer;width:20px;height:20px;float:left;;margin:1px;padding:0px;}\r\n";
        stylestr += "#Ice_calendar-control{height:25px;z-index:9998;}\r\n";
        stylestr += "#Ice_calendar-today{cursor:pointer;float:left;width:63px;height:20px;line-height:20px;margin:1px;text-align:center;background:#BAE2F8;}"
        stylestr += "</style>";
    ...
    {
        this.InsertHTML("Ice_calendar-year", yy + " 年");
        this.InsertHTML("Ice_calendar-month", mm + " 月");
    },
    ...
    SetDay: function(yy, mm)   //主要的写程序**********
    {
        ...
        if (window.parent.location.href != window.location.href) {
            for (var i_f = 0; i_f < window.parent.frames.length; i_f++) {
                if (window.parent.frames[i_f].location.href == window.location.href) { L_DateLayer_Parent = window.parent.frames[i_f]; }
            }
        }
        ...
        for (i = 0; i < 39; i++) {
            var da = this.GetDateLayer().document.getElementById("Ice_calendar-day_" + i + "");
            var month, day;
            if (this.L_WDay[i] != "") {
                if (i < firstday) {
                ...
                da.style.background = (yy == new Date().getFullYear() && month == new Date().getMonth() + 1 && day == new Date().getDate()) ? "#86D700" : "#e0e0e0";
                if (this.InputDate != null) {
                    if (yy == this.InputDate.getFullYear() && month == this.InputDate.getMonth() + 1 && day == this.InputDate.getDate()) {
                        da.style.background = "#75B8FF";
                    }
                }
            }
        }
    ...
    SetDate: function() {
        ...
        if (document.all) {
            this.GetDateLayer().document.getElementById("Ice_calendar").style.width = "160px";
            this.GetDateLayer().document.getElementById("Ice_calendar").style.height = "180px"
        }
        else {
            this.GetDateLayer().document.getElementById("Ice_calendar").style.width = "154px";
            this.GetDateLayer().document.getElementById("Ice_calendar").style.height = "180px"
...
var IceCalendar = new Ice_calendar();
IceCalendar.NewName = "IceCalendar";
document.onclick = function(e) {
    e = window.event || e;
    var srcElement = e.srcElement || e.target;
    IceCalendar.CloseLayer(srcElement);
}